...
class PerBasinBlockH5Dataset(Dataset):
    ...
    def __init__(
        ...
        self.static_cache_size = max(1, int(static_cache_size))
        self._h5_handles: Dict[str, h5py.File] = {}
        self._static_cache: "OrderedDict[str, Dict[str, np.ndarray]]" = OrderedDict()
        ...
        # Training/validation now always load per-basin H5 strictly by qcsv basin order.
        # manifest.csv is intentionally ignored here; each basin must map to
        # <h5_path>/<basin_id>.h5.
        self.missing_h5_basins: List[str] = []
        if basins is None:
            raise ValueError(
                "PerBasinBlockH5Dataset now requires basin ids from qcsv; "
                "pass the qcsv basin_id sequence via the 'basins' argument."
            )
        basin_files = []
        seen_basins = set()
        for basin in basins:
            ...
            basin_files.append((basin_id, self.h5_dir / f"{basin_id}.h5"))
        ...
        if self.missing_h5_basins:
            ...
            print(
                f"[warn] skipped {len(self.missing_h5_basins)} basin(s) from qcsv because matching H5 files "
                f"were not found under {self.h5_dir}: {preview}{suffix}"
            )

        if not self.samples:
            raise RuntimeError("No per-basin block samples with usable targets found.")
    ...
    def _get_static_arrays(self, basin_id: str, f: h5py.File) -> Dict[str, np.ndarray]:
        cached = self._static_cache.get(basin_id)
        if cached is not None:
            self._static_cache.move_to_end(basin_id)
            return cached
        cached = {
        ...
        self._static_cache[basin_id] = cached
        if len(self._static_cache) > self.static_cache_size:
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        static_arrays = self._get_static_arrays(basin_id, f)
        ...
        item = {
            ...
            "x_stat_base": torch.from_numpy(static_arrays["x_stat_base"]),
            ...
            "dist_m": torch.from_numpy(static_arrays["dist_m"]),
            "area_m2": torch.from_numpy(static_arrays["area_m2"]),
            "fraction": torch.from_numpy(static_arrays["fraction"]),
        }
