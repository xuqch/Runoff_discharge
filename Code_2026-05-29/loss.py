...
def _extract_finite_q_pairs(
    ...
    *,
    ...
    if int(q_valid.sum().item()) < min_count:
        return None, None

    obs_all = meta["q_true"].to(device, non_blocking=True).reshape(-1)
    sim_all = q_pred.reshape(-1)
    finite_mask = q_valid & torch.isfinite(obs_all) & torch.isfinite(sim_all)
    if int(finite_mask.sum().item()) < min_count:
        return None, None

    obs = obs_all[finite_mask]
    sim = sim_all[finite_mask]
    if obs.numel() < min_count or sim.numel() < min_count:
        return None, None
    return obs, sim


def _sanitize_scalar_loss(loss: torch.Tensor) -> torch.Tensor:
    if torch.isfinite(loss.detach()).all():
        return loss
    return _zero_scalar(loss.device)
...
class MFMLoss(nn.Module):
    ...
    def forward(
        ...
        for q_pred, meta in zip(q_pred_list, basin_meta):
            obs, sim = _extract_finite_q_pairs(q_pred, meta, min_count=3)
            if obs is None or sim is None:
                continue

            mfm = self._mfm_1d(sim, obs)
            if torch.isnan(mfm):
                continue
            losses.append(1.0 - mfm)

        if not losses:
            return _zero_scalar(q_mean_global.device)
...
class NSELoss(nn.Module):
    ...
    def forward(
        ...
        for q_pred, meta in zip(q_pred_list, basin_meta):
            device = q_pred.device
            obs, sim = _extract_finite_q_pairs(q_pred, meta, min_count=1)
            if obs is None or sim is None:
                continue

            q_std_vec = meta["q_std_loss"].to(device, non_blocking=True).reshape(-1)
            q_std_vec = q_std_vec[torch.isfinite(q_std_vec)]
            if q_std_vec.numel() == 0:
                continue
            q_std_loss = torch.clamp(q_std_vec[0], min=self.eps)
            ...
            loss_i = torch.mean(high_weight * ((sim - obs) / (q_std_loss + self.eps)) ** 2)
            if torch.isfinite(loss_i.detach()).all():
                losses.append(loss_i)
            del q_std_loss, q90, high_weight, obs, sim, q_std_vec, scaled, loss_i
        if not losses:
            return _zero_scalar(q_mean_global.device)
        return torch.stack(losses).mean()
...
class _BaseHydroLoss(nn.Module):
    ...
    def _balance_term(self, outputs: Dict[str, List[torch.Tensor]], basin_meta: List[Dict], device: torch.device) -> torch.Tensor:
        if self.w_balance <= 0.0 or self.balance_module is None:
            return _zero_scalar(device)
        return _sanitize_scalar_loss(self.balance_module(outputs["runoff"], basin_meta))
...
class HydroLoss(_BaseHydroLoss):
    ...
    def forward(
        ...
        loss_q = _sanitize_scalar_loss(loss_q)
        loss_mb = _sanitize_scalar_loss(loss_mb)
        total = self.w_q * loss_q + self.w_balance * loss_mb
        total = _sanitize_scalar_loss(total)
...
class HydroMFMLoss(_BaseHydroLoss):
    ...
    def forward(
        ...
        if self.w_peak > 0.0:
            ...
            loss_peak = _zero_scalar(loss_q.device)

        loss_q = _sanitize_scalar_loss(loss_q)
        loss_mb = _sanitize_scalar_loss(loss_mb)
        loss_peak = _sanitize_scalar_loss(loss_peak)
