...
class EALSTM(nn.Module):
    PRECOMPUTE_GATE_POSITIONS_LIMIT = 1_200_000
    ...
    def __init__(
        ...
        precompute_gate_positions_limit: int = PRECOMPUTE_GATE_POSITIONS_LIMIT,
    ):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.precompute_inputs = bool(precompute_inputs)
        self.precompute_time_chunk = int(precompute_time_chunk)
        self.precompute_gate_positions_limit = int(precompute_gate_positions_limit)
    ...
    def _precompute_lin_x(self, x_dyn: torch.Tensor) -> Optional[torch.Tensor]:
        ...
        # Large basins can make the full [P, T, 3H] tensor too expensive.
        # Fall back to the older step-wise path when the temporal batch is large.
        gate_positions = int(x_dyn.shape[0]) * int(x_dyn.shape[1])
        limit = self.precompute_gate_positions_limit
        if limit is not None and int(limit) > 0 and gate_positions > int(limit):
            return None
    ...
    def forward(
        ...
        h_seq: Optional[torch.Tensor] = (
            x_dyn.new_empty(batch_size, seq_len, self.hidden_dim) if return_sequence else None
        )
        ...
        out = h_seq if return_sequence else h_last
        if return_state:
            return out, (h, c)
        return out, None
...
class PixelWiseRunoffGenerator(nn.Module):
    def __init__(
        ...
        precompute_inputs: bool = True,
        precompute_time_chunk: int = 0,
        precompute_gate_positions_limit: int = EALSTM.PRECOMPUTE_GATE_POSITIONS_LIMIT,
    ):
        super().__init__()
        self.ealstm = EALSTM(
            dyn_dim=dyn_dim,
            ...
            precompute_gate_positions_limit=precompute_gate_positions_limit,
        )
...
class DifferentiableMuskingumRoutingLayer(nn.Module):
    ...
    def _adaptive_max_lag(
        ...
        c2_safe = torch.nan_to_num(c2.detach(), nan=0.0, posinf=1.0 - 1e-6, neginf=0.0)
        c2_safe = c2_safe.clamp(0.0, 1.0 - 1e-6)
        ...
        tail_steps = torch.nan_to_num(tail_steps, nan=1.0, posinf=float(self.max_lag_limit), neginf=1.0)
        tail_steps = tail_steps.clamp_min(1.0)
        max_tail_steps = tail_steps.max()
        if not torch.isfinite(max_tail_steps):
            max_needed = 2
        else:
            max_needed = int(torch.ceil(max_tail_steps).item()) + 2
...
class HydroAIBasin(nn.Module):
    ...
    def _generator_forward(self, x_dyn: torch.Tensor, x_stat: torch.Tensor) -> torch.Tensor:
        return self.generator(x_dyn, x_stat, return_state=False)[0]

    def _generator_forward_sequence(self, x_dyn: torch.Tensor, x_stat: torch.Tensor) -> torch.Tensor:
    ...
    def generate_runoff(
        ...
        for start in range(0, total, chunk_size):
            ...
            if self.training and use_checkpoint:
                ...
                runoff_chunk = self._generator_forward(x_dyn_chunk, x_stat_chunk)
    ...
    def generate_runoff_sequence(
        ...
        for start in range(0, total, chunk_size):
            ...
            if self.training and use_checkpoint:
                runoff_chunk = checkpoint(
                    self._generator_forward_sequence,
                    x_dyn_chunk,
                    x_stat_chunk,
                    use_reentrant=False,
                )
            else:
                runoff_chunk = self._generator_forward_sequence(x_dyn_chunk, x_stat_chunk)
