...
def read_basin_table(q_file: str | os.PathLike[str]) -> pd.DataFrame:
    q_path = Path(q_file)
    basins_df = pd.read_csv(q_path, dtype={"basin_id": str, "station_id": str})
    if "basin_id" not in basins_df.columns:
        raise KeyError(
            f"{q_path} must contain a 'basin_id' column. "
            f"Available columns: {list(basins_df.columns)}"
    ...
    if "station_id" in basins_df.columns:
        basins_df["station_id"] = basins_df["station_id"].fillna("").astype(str).map(lambda value: value.strip())
    else:
        basins_df["station_id"] = ""
    return basins_df
